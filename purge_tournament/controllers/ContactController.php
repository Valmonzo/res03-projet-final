...
class ContactController extends AbstractController {
    ...
    public function newMessage(array $post) : void {
        ...
        && !empty($post['contactMessage'])
            ...
            $contactName = filter_var($post['contactName'], FILTER_SANITIZE_STRING);
            $contactEmail = filter_var($post['contactEmail'], FILTER_SANITIZE_EMAIL);
            $contactMessage = filter_var($post['contactMessage'], FILTER_SANITIZE_STRING);
            $messageToAdd = new Contact($contactName, $contactEmail, $contactMessage);
