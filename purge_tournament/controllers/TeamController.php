...
class TeamController extends AbstractController
{
    ...
    {
        // Je vérifie si le formulaire est bien rempli
        if (
            !empty($post["teamName"]) &&
            !empty($post["teamP1"]) &&
            !empty($post["teamP2"]) &&
            ...
            !empty($post["coach"]) &&
            !empty($post["logo"]) &&
             // Si c'est bien le bon formulaire
            $post["formName"] === "team-edit"
        ) {
            // Je créer une instance de Team pour utiliser l'update du manager
            ...
            header(
                "Location: /res03-projet-final/purge_tournament/admin/teams"
            );
        } else {
    ...
    public function create(array $post): void
    {
        if (
            ...
            header(
                "Location: /res03-projet-final/purge_tournament/admin/teams"
            ); // Je redirige vers la liste des teams
        } else {
            // Si le formulaire n'est pas bien rempli
            $this->render(
                "teams/create",
                ["error" => "Merci de bien remplir tous les champs"],
                "private"
            ); // Je render la même page avec une erreur
        }
    }
